...
- **Loop-until-dry contradiction termination decision table** (`CONTRADICTION_K` = 2 consecutive dry rounds, `CONTRADICTION_CAP` = 5 hard iteration cap). Each round dispatches one agent per cluster (clustered by `module` / `problem_type` / `tags`). `found_new` means a newly-discovered contradicting **doc-pair** — a round that only surfaces additional facets (dimensions) of an already-seen pair is "dry" (discovery has converged), so a thorough classifier is not penalized into a never-quiescent loop; all facets are still collected for the report:
