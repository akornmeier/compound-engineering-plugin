...
describe("parsePlanUnits — Files keeps well-known extensionless files", () => {
  // Extensionless repo-root files (Dockerfile, Makefile, LICENSE, ...) are valid
  // repo-relative paths per ce-plan, but bare identifiers/tool names are not.
  const PLAN = `## Implementation Units

### U1. Container + license bookkeeping

**Goal:** Touch some extensionless files.

**Files:**
- \`Dockerfile\` (new)
- \`Makefile\` (modified)
- \`LICENSE\` (new); generated by the \`Read\` tool, no \`import\` of \`fs\`
- \`Procfile\` (new)
- \`.gitignore\` (modified)
...
`
  ...
  test("known extensionless files and dotfiles are kept", () => {
    expect(u.files.all).toEqual([
      "Dockerfile",
      "Makefile",
      "LICENSE",
      "Procfile",
      ".gitignore", // dotfile passes via the extension rule
    ])
  })

  test("capitalized tool names and bare identifiers in the same Files line are still skipped", () => {
    // Read/import/fs sit on the LICENSE bullet line; the per-token filter drops them.
    expect(u.files.all).not.toContain("Read") // tool name, not in the allowlist
    expect(u.files.all).not.toContain("import")
    expect(u.files.all).not.toContain("fs")
  })
})
