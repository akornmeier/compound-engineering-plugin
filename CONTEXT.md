...
**Drift rate**:
The **Signal gate**'s reading: the fraction of a plan's *attempted* tasks (`done + drifted`) that *drifted* (claimed done but the repo diverged, or claimed done but not actually done). The denominator is the attempted-and-verifiable set, **not** total tasks — never-started (`remaining`) and not-statically-settleable (`unverifiable`) tasks are excluded, so the rate measures rework rather than plan-completion stage (`ce-verify-work` ships this operational definition). **Derived at read-time** by aggregating captured drift readings + session history — never stored as a number (that would reopen the out-of-scope task-ledger). The proxy for **rework/churn**. *(Capture now shipped: `ce-verify-work` writes a durable per-run **Drift event** to `docs/drift-events/` — the aggregation's input now exists; the aggregation, the Signal gate, and the threshold `T` remain deferred.)*
