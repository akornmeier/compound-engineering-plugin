...
- **Merge policy:** All changes to `main` go through pull requests. Direct pushes and direct merges are not allowed. Note: this is policy, not yet machine enforcement — verified live 2026-06-12, the `main` rulesets cover only deletion protection, non-fast-forward, and Copilot review; the `test` status check is NOT currently a required check. Making `test` required is the standing recommendation (see the ruleset recommendations in `plugins/compound-engineering/skills/ce-learning-sweep/references/trigger-recipe.md`). The direct path bypasses `release:validate`, the test suite, and PR title validation — past direct merges have caused version drift requiring multi-PR recovery (see `docs/solutions/workflow/release-please-version-drift-recovery.md`).
